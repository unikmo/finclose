...
export const PAYROLL_RULE_PACK_MU = {
  id: 'MU-2026-27-CUMULATIVE-PAYE-CSG-NSF-PRGF-DRAFT-V1',
  ...
  limitations: [
    ...
    'NOT IMPLEMENTED AT ALL in v1, rejected outright: director/board fees (flat 15%/20% PAYE), nonresident-employee-specific PAYE treatment beyond zeroing EDF relief, daily-paid-worker exclusion, low-monthly-emoluments PAYE exclusion (Rs38,462 threshold), and the individual Fair Share Contribution (15% above Rs12m/13 per period). This engine prepares payroll and accounting outputs only — it does not file with the MRA.',
...
export function calculateMuPayroll(input: MuPayrollRunInput): MuPayrollRunResult {
  ...
  const employees = input.employees.map(employee => {
    ...
    if (employee.resident_status === 'RESIDENT' && (typeof employee.annual_edf_reliefs_total !== 'number' || employee.annual_edf_reliefs_total < 0)) {
    ...
    const paye = Math.max(0, money(cumulativeTax - payeWithheldYtdBefore));
    const payeWithheldYtdAfter = money(payeWithheldYtdBefore + paye);
...
export function payrollEngineSelfTestMU() {
  ...
  return { ok, b500000, b500001, b12m, period1, period2, csgLow, csgHigh, prgfStandard, prgfExempt };
}
