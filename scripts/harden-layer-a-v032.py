...
p = Path('lib/finance-cycle-engine.ts')
s = p.read_text()
old = '''  let cycleId = baseCycleId;
...
'''
if old not in s:
    raise SystemExit('expected reopen-id block not found')
s = s.replace(old, new, 1)
old = '''    bookkeeping_batch_id: bookkeepingBatch.bookkeeping_batch_id,
    created_at: now,
    updated_at: now
'''
new = '''    bookkeeping_batch_id: bookkeepingBatch.bookkeeping_batch_id,
...
'''
if old not in s:
    raise SystemExit('expected close record lineage block not found')
s = s.replace(old, new, 1)
old = '''    bookkeeping_batch_id: bookkeepingBatch.bookkeeping_batch_id,
...
'''
if old not in s:
    raise SystemExit('expected cycle lineage block not found')
s = s.replace(old, new, 1)
s = s.replace("execution_boundary: 'NO_PAYMENT_NO_FILING_NO_EXTERNAL_POSTING_NO_PERIOD_LOCK'", "execution_boundary: 'NO_PAYMENT_NO_FILING_NO_EXTERNAL_POSTING; INTERNAL_PERIOD_LOCK_AFTER_LAYER_A_APPROVAL'", 1)
old = '''    [`finclose_bookkeeping_batches/${bookkeepingBatch.bookkeeping_batch_id}/finance_cycle_id`]: cycleId,
...
'''
new = '''    [`finclose_bookkeeping_batches/${bookkeepingBatch.bookkeeping_batch_id}/latest_finance_cycle_id`]: cycleId,
    [`finclose_bookkeeping_batches/${bookkeepingBatch.bookkeeping_batch_id}/source_payroll_run_id`]: payrollRun.payroll_run_id,
...
'''
if old not in s:
    raise SystemExit('expected bookkeeping lineage writes not found')
s = s.replace(old, new, 1)
p.write_text(s)
